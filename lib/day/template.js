...
const parseJourney = (params) => (journey) => {
	...
	return {
		...
		currency: currency(journey.price.currency),
		...
		link: journey.price.url || buildLink(params.origin, params.destination, moment(journey.legs[0].departure).tz(timezone), params) || '#'
	}
}
...
const price = (journey) => [
	html.span('.columnLong', journey.price.euros+","+journey.price.cents+journey.currency),
	html.span('.columnShort', Math.round(+journey.price.euros + (+journey.price.cents/100))+journey.currency)
]
