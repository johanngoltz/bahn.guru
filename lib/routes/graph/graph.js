...
const addShorthandAttributesToJourney = journey => ({
	...
	price: { ...journey.price, ...helpers.formatPrice(journey.price.amount) }
})
		...
		.then(inAndOutbundJourneysPerDay => {
			...
			for (let i = 0; i < inAndOutbundJourneysPerDay.length;) {
				...
				if (journeyCombinations.outbound && journeyCombinations.inbound) {
					const combinedAmount = journeyCombinations.outbound.price.amount + journeyCombinations.inbound.price.amount
					...
					journeyCombinations.combined = {
						price: { amount: combinedAmount, ...helpers.formatPrice(combinedAmount) },
						// calculation doesnt seem to work as intended when the DST switch is between arrival & departure
