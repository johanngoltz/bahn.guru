...
const createChartEntry = (minAmount, maxAmount, date, outbound, inbound, combined) =>
	html.div('.day', [
		html.div({ class: 'bar-with-overlay', style: `height: ${combined.price / maxAmount * 100 - 10}%` }, [
			html.div('.bar' + (minAmount === combined.price ? ' .cheapest' : '')),
			html.div('.overlay', [
				html.div('.price', `${Math.round(combined.price, 2)}€`),
				html.div('.duration', [
			...
			html.div({
				class: 'overlay day-connector',
				style: `width: ${combined.daysStay * 100 + 50}%`
			})
		]),
