...
const createChartEntry = (minAmount, maxAmount, date, outbound, inbound, combined) =>
	html.div('.day' + (date.raw.isoWeekday() > 5 ? '.weekend' : ''), [
...
const graph = (api, data) => {
	...
	const minAmount = Math.min(...combinedAmounts)
	const maxAmount = Math.max(...combinedAmounts)
	const createChartEntryInBounds = createChartEntry.bind(null, minAmount, maxAmount)
	for (let { date, combinations } of data.output) {
		...
		if (!outbound || !outbound.price || !inbound || !inbound.price) {
			...
			bars.push(createChartEntryInBounds(date.date, outbound, inbound, combined))
		}
	}
	const minMaxMarkers = html.div({
		class: 'day',
		style: 'margin-bottom:1rem;'
	}, [html.span({ style: `height: 100%` }, `${maxAmount}`), html.span({ style: `height: ${100 * minAmount / maxAmount}%` }, `${minAmount}`)])
	return html.div('#graph', [minMaxMarkers, ...bars])
}
