...
const createChartEntry = (minAmount, maxAmount, date, outbound, inbound, voyageCombinedPrice) =>
	html.div('.day', [
		html.div('.label', date.date.formatted),
		html.div(
			{
				class: 'bar' + (minAmount === outbound.price.amount ? ' cheapest' : ''),
				style: `height: ${voyageCombinedPrice / maxAmount * 100 - 10}%`
			}, [

			]),
		html.div('.overlay', [
			html.div('.price', `${Math.round(voyageCombinedPrice, 2)}€`),
			html.div('.duration', [
...
// html.span('', `${moment(forJourney.legs[0].departure).format('D.M. HH:mm')} — ${moment(forJourney.legs[forJourney.legs.length - 1].arrival).format('D.M. HH:mm')}, ${Math.round(voyageCombinedPrice, 2)}€`)

const graph = (api, data) => {
	if (!data) return html.span()
	const result = []
	const bars = []
	const combinedAmounts = data.output.map(o => o.combinations.combined.price)
	...
	for (let { date, combinations } of data.output) {
		...
		if (!outbound || !outbound.price || !inbound || !inbound.price) {
			...
			bars.push(createBar(date, outbound, inbound, combined.price))
		}
	}
